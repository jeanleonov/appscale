...
class SolrQueryParser():
  ...
  def get_solr_query_string(self, query):
    ...
    if len(query) > 0:
      ...
      #query_string += "+AND+" + urllib.quote_plus(self.__create_query_string(query_tree), '+')
    ...
    logging.debug(u"UNQUOTED STRING: {0}".format(urllib.unquote(query_string)))
    return query_string
  ...
  def __create_query_string(self, query_tree):
    ...
    if query_tree.getType() == QueryParser.CONJUNCTION:
      ...
      if field.getType() == QueryParser.GLOBAL:
        ...
        escaped_value = match #self.__escape_chars(match) #TODO
